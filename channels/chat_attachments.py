...
def derive_attachment_download_url(ws_url, attachment_id):
    parsed = urlsplit(ws_url)
    if parsed.scheme not in {"ws", "wss"} or not parsed.netloc:
        raise AttachmentDownloadError("invalid WS_URL for attachment download")
    if parsed.username is not None or parsed.password is not None:
        raise AttachmentDownloadError("WS_URL credentials are not supported")
    if not parsed.path.endswith("/ws"):
        raise AttachmentDownloadError("WS_URL must end with /ws for attachment download")

    http_scheme = "https" if parsed.scheme == "wss" else "http"
    path = f"{parsed.path[:-3]}/attachments/{quote(str(attachment_id), safe='')}"
    return urlunsplit((http_scheme, parsed.netloc, path, "", ""))
...
def download_attachment(
    ...
    *,
    ...
    try:
        ...
        raise AttachmentDownloadError(f"attachment endpoint request failed: {exc}") from exc
    ...
    try:
        ...
        temporary.unlink(missing_ok=True)
        raise AttachmentDownloadError(f"attachment object download failed: {exc}") from exc

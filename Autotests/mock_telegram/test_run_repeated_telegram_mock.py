...
from helpers import (
    Checker, dexec, dexec_root, make_prompt, wait_for_file,
)
from tg_helpers import tg_send_prompt
...
def test_run_repeated_telegram_mock(llm, tg):
    with Checker("run repeated script (telegram mock)", cleanup_dirs=[TARGET_DIR]) as c:
        print(f"\n=== OmegaClaw: run 10x telegram mock (run-id {c.run_id}) ===",
              flush=True)
        ...
        c.step("send prompt via Telegram with mocked response")
        prompt = make_prompt(
            c.run_id,
